...
def parse_samples():
    ...
    time_matrix = {}
    samples_filepaths = sorted(glob.glob("example/time/dataset/*.java"))
    #samples_filepaths = sorted(glob.glob("example/test/runtime-attr/*.java"))

    for filepath in samples_filepaths:
        print(str(filepath))
        time_matrix[filepath] = []
        for i in range(21):
            start = time.time()
            #print(str(filepath))
            ...
            sample_filename = re.search(r"example\/time\/dataset\/(.+\.java)", str(filepath)).group(1)
            #sample_filename = re.search(r"example\/test\/runtime-attr\/(.+\.java)", str(filepath)).group(1)
            ...
            with open( "example/time/ast/" + sample_filename.replace("java", "txt"), "w+") as file:
                file.write(tree_string)
            parse_ast("example/time/ast/" + sample_filename.replace("java", "txt"))
            end = time.time()
            time_matrix[filepath].append(end - start)
    #write_csv("results/time.csv", time_matrix, samples_filepaths)
    with open("results/time.csv", "w") as f:
        w = csv.writer(f, samples_filepaths)
        w.writerow(samples_filepaths)
        for k in samples_filepaths:
            #w.writerow({field: matrix[k].get(field) or k for field in fields})
            w.writerow(time_matrix[k])
    convert_excel()


def parse_ast(filepath):
    ast_name = re.search(r"example\/time\/ast\/(.+)\.txt", str(filepath)).group(1)
